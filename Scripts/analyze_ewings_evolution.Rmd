...
- For each date, evolutionary replicates 6-8 are controls. The mean IC50 value between these three replicates is extracted for each drug, termed the `control_value`
...
IC50_data_A673 <- readRDS(here("Data", "original_ic50_A673.rds"))
IC50_data_TTC466 <- readRDS(here("Data", "original_ic50_TTC466.rds"))
IC50_data_A673_main <- IC50_data_A673[ , !colnames(IC50_data_A673) %in% c("Paz", "Vin", "NaThio")]
IC50_data_TTC466_main <- IC50_data_TTC466[ , !colnames(IC50_data_TTC466) %in% c("Paz", "Vin", "NaThio")]
IC50_change_data_A673 <- readRDS(here("Data", "logchange_ic50_A673.rds"))
IC50_change_data_TTC466 <- readRDS(here("Data", "logchange_ic50_TTC466.rds"))
IC50_change_data_seq <- readRDS(here("Data", "logchange_ic50_seq.rds"))
expr_data_SARC_tcga <- readRDS(here("Data", "expr_data_sarcoma_tcga.rds"))
expr_data_lab <- readRDS(here("Data", "expr_data_lab.rds"))
```


# Heatmaps of IC50 changes

```{r plot_heatmaps}
make_ic50_heatmap <- function(ic50_data, cell_line){
  ic50_data[ic50_data > 3] <- 3
  ic50_data[ic50_data < -3] <- -3
  print(pheatmap(ic50_data, treeheight_row = 0, treeheight_col = 0,
...
colnames(IC50_change_data_A673)
reps_A673 <- substr(row.names(IC50_change_data_A673), 7, 8)
heatmap_ic50_A673 <- IC50_change_data_A673[reps_A673 %in% c("R1", "R3", "R4", "R5"), 
                                    !(colnames(IC50_change_data_A673) %in% c("NaThio", "Paz", "Vin"))]
heatmap_ic50_A673["TimeJ_R5", "Ola"] <- NA 


reps_TTC466 <- substr(row.names(IC50_change_data_TTC466), 7, 8)
heatmap_ic50_TTC466 <- IC50_change_data_TTC466[reps_TTC466 %in% c("R1", "R2", "R3", "R4", "R5"), 
                                        !(colnames(IC50_change_data_TTC466) %in% c("NaThio", "Paz", "Vin"))]
heatmap_ic50_TTC466[rownames(heatmap_ic50_TTC466) %in% c("TimeJ_R3", "TimeK_R3"), 
                    colnames(heatmap_ic50_TTC466) %in% c("Ola", "Etp", "Doxo", "ActD")] <- NA


heatmap_ic50_seq <- IC50_change_data_seq[ , !(colnames(IC50_change_data_TTC466) %in% c("NaThio", "Paz", "Vin"))]
...
waterfall.plot <- function(df, column.name, cell_line, metric, times){
  ...
  if(metric == "IC50)"){
    metric <- "IC50 (mM)"
  } # Adds unit for only EC50/IC50, because AUC should not have units
  waterfall <- ggplot(df, aes(x=labels, y = UQ(sym(column.name)), fill = UQ(sym(column.name)))) +
    ...
    theme_bw() +
          ...
          scale_fill_gradient2(name = "log2(\u0394IC50)",
...
for (drug in colnames(IC50_change_data_seq)) { 
                                        # Removes Time column from waterfall plotting
  IC50_change_data_seq["I1", "Vin"] <- NA
  waterfall.plot(df = IC50_change_data_seq, column.name = drug, 
                 cell_line = "A673", metric = "EC50", times = timepoints)
  # Make waterfall plot for change in IC50 for samples sent to sequencing against each drug
  filename <- paste0("../Plots/Waterfall/RNA_Seq/", drug, "_EC50.png")
  cat("![](",filename,")")
}

# print(rownames(IC50_change_data_seq))

```

# Point-Range Plots of IC50 over time

```{r point_range_plot, message=FALSE}

plot_point_range <- function(ic50_df, cell_line=c("A673", "A673_Complete", "TTC466", "TTC466_Complete")){
  # ic50_df: data frame where rows contain individual replicates at individual time points
  ...
  if(cell_line %in% c("A673", "A673_Complete")){
    ...
    ic50_df <- melt(ic50_df)
    ic50_df$rep_group <- rep(c("Experimental", "Experimental", "Experimental", "Experimental", 
                                 "Control", "Control", "Control"), 12)
    ic50_df$time <- factor(c(rep("Pre-Tx", 7),
                        rep("VDC1", 7), 
                        rep("EC1", 7), 
                        rep("VDC2", 7),
                        rep("EC2", 7),
    ...
    if(cell_line=="A673"){
      add_rows <- ic50_df[c(75:77, 159:161, 243:245, 327:329, 411:413,
                            495:497, 579:581, 663:665, 747:749), ]
      add_rows$time <- rep("EC5", nrow(add_rows))
      ic50_df <- rbind(ic50_df, add_rows)
    }
  ...
  if(cell_line %in% c("TTC466", "TTC466_Complete")){
    ...
    ic50_df["TimeE_R3", "NaThio"] <- NA
    ic50_df["TimeE_R8", "Paz"] <- NA
    ic50_df["TimeA_R7", "Paz"] <- NA
    ic50_df <- melt(ic50_df)
    ic50_df$rep_group <- rep(c("Experimental", "Experimental", "Experimental", 
                                 "Experimental", "Experimental", 
                                 "Control", "Control", "Control"), 11)
    ic50_df$time <- factor(c(rep("Pre-Tx", 8),
                      rep("VDC1", 8), 
                      rep("EC1", 8), 
                      rep("VDC2", 8),
                      rep("EC2", 8),
  ...
  ic50_df$variable <- factor(ic50_df$variable, ordered = TRUE, 
                            levels = c("ActD", "Cyclo", "Doxo", "Etp", "Ola", 
                                        "SAHA", "SN38", "SP", "TMZ", "Vin", 
                                        "NaThio", "Paz"))
  ic50_df %>% 
    group_by(variable, rep_group, time) %>%
    summarise(mean_ic50 = mean(value, na.rm = TRUE), 
              max_ic50 = max(value, na.rm = TRUE), 
              min_ic50 = min(value, na.rm = TRUE)) -> ic50_summary
  # Plot figure
  png(filename = paste0("../Plots/pointrange_", cell_line, ".png"),
      width = 1900, height = 1400, res = 200)
  print(ggplot(ic50_summary, aes(x=time, y=mean_ic50, ymin=min_ic50, ymax=max_ic50,
...
IC50_data_A673_edit <- IC50_data_A673_main
IC50_data_A673_edit["TimeJ_R5", "Ola"] <- NA
plot_point_range(IC50_data_A673_edit, cell_line = "A673")
plot_point_range(IC50_data_A673, cell_line = "A673_Complete")

IC50_data_TTC466_edit <- IC50_data_TTC466_main
IC50_data_TTC466_edit[row.names(IC50_data_TTC466_edit) %in% c("TimeJ_R3", "TimeK_R3"),
                      colnames(IC50_data_TTC466_edit) %in% c("ActD", "Ola", "Etp", "Doxo")] <- NA
plot_point_range(IC50_data_TTC466_edit, cell_line = "TTC466")
plot_point_range(IC50_data_TTC466, cell_line = "TTC466_Complete")

```
Figure 2
![](../Plots/pointrange_A673.png)

Supplementary Figure 2
![](../Plots/pointrange_A673 Complete Data.png)

Supplementary Figure 4
![](../Plots/pointrange_TTC466 Complete Data.png)
...
high_low_subset <- function(drug_data, drug, percent){
  ...
  median_ic50 <- median(drug_data[ , drug])
...
for (drug_name in colnames(IC50_change_data_seq)){
  IC50_change_data_seq["I1", "Vin"] <- NA
  print(drug_name)
  write(drug_name, file = "../DE_Results/all_DE_results_latex.txt", append = TRUE)
  run_de_analysis(drug_data = IC50_change_data_seq,
